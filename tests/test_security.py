...
class TestPathSecurity:
    ...
    def test_path_injection_prevention_dots(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            for malicious_path in malicious_paths:
                ...
                assert (
                    "error" in result
                ), f"Path injection should be blocked for: {malicious_path}"

                # Test directory structure
                result = get_directory_structure(Path(malicious_path))
                assert result is None or "error" in str(
                    result
                ), f"Directory access should be blocked for: {malicious_path}"
    ...
    def test_path_injection_prevention_absolute_paths(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            for malicious_path in malicious_paths:
                ...
                assert (
                    "error" in result
                ), f"Absolute path access should be blocked for: {malicious_path}"
    ...
    def test_special_characters_in_paths(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            for special_path in special_chars:
                ...
                assert isinstance(
                    result, dict
                ), f"Should return dict for special path: {special_path}"
                # Should either succeed or fail gracefully with error
                assert (
                    "content" in result or "error" in result
                ), f"Should have content or error for: {special_path}"
    ...
    def test_unicode_filename_handling(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            assert isinstance(
                result, list
            ), "Should return list even with unicode files"
            ...
            for unicode_file in unicode_files:
                if (temp_project_dir / unicode_file).exists():
                    ...
                    assert isinstance(
                        content_result, dict
                    ), f"Should handle unicode file: {unicode_file}"
    ...
    def test_directory_boundary_enforcement(self, temp_project_dir):
        ...
        with tempfile.TemporaryDirectory() as outside_dir:
            ...
            with patch(
                ...
                assert (
                    "error" in result
                ), "Should not access files outside project via relative path"
...
class TestEncodingRobustness:
    ...
    def test_different_encodings(self, temp_project_dir):
        ...
        for encoding, content in encodings_to_test:
            ...
            with patch(
                ...
                assert (
                    "content" in result or "error" in result
                ), f"Should process {encoding} file"
    ...
    def test_binary_file_detection(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            # Content might be decoded with replacement characters due to
            # errors='ignore'
    ...
    def test_malformed_unicode(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            assert (
                "content" in result or "error" in result
            ), "Should process malformed unicode file"
