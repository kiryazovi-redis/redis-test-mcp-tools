...
class TestDetectFrameworkContext:
    ...
    def test_detect_unittest_framework_with_testcase_inheritance(self, temp_project_dir):
...
'''
        ...
        result = _detect_framework_context(str(test_file), func_node, func_info, file_imports)
...
''')
    ...
    def test_analyze_test_files_handles_syntax_errors_gracefully(self, temp_project_dir):
...
''')
        ...
        expected_keys = [
            'testing_frameworks', 'framework_usage', 'common_fixtures',
            'common_markers', 'mocking_patterns', 'assertion_patterns',
            'setup_patterns', 'parametrization_patterns', 'unittest_patterns',
            'pytest_patterns'
        ]
...
''')
        ...
        expected_keys = [
            'untested_functions', 'untested_classes', 'untested_files',
            'analysis_summary'
        ]
...
''')
        ...
        assert '_private_function' not in function_names  # Private functions are excluded
...
class TestSuggestTestCasesAdvanced:
    ...
    def test_suggest_test_cases_with_complex_function_signatures(self, temp_project_dir):
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            mock_config.project_root = temp_project_dir
            result = suggest_test_cases(str(source_file), function_name="complex_function")
        ...
        parameter_tests = [name for name in test_names if any(param in name for param in ['str_param', 'int_param', 'list_param', 'dict_param'])]
        assert len(parameter_tests) > 0
        
        # Check for exception test based on docstring
        assert any('raises_exception' in name or 'exception' in name for name in test_names)
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            mock_config.project_root = temp_project_dir
            result = suggest_test_cases(str(source_file), function_name="async_function")
        ...
        async_tests = [t for t in result['test_suggestions'] if 'async' in t.get('test_name', '').lower()]
        if async_tests:
            ...
            assert any('basic' in name for name in test_names)
            # And verify that the function's async nature can be suggested through other test types
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            mock_config.project_root = temp_project_dir
            result = suggest_test_cases(str(source_file), class_name="ComplexCalculator")
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.get_test_patterns') as mock_get_patterns:
...
''')
        ...
        expected_keys = [
            'file_path', 'framework', 'recommended_framework', 'test_suggestions'
        ]
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            mock_config.project_root = temp_project_dir
            result = suggest_test_cases(str(source_file), function_name="target_function")
        
        assert len(result['test_suggestions']) > 0
        
        # Should have suggestions for the target function
        function_tests = [s for s in result['test_suggestions'] 
                         if 'target_function' in s['test_name']]
...
''')
        ...
        class_tests = [s for s in result['test_suggestions'] 
                      if 'TargetClass' in s['test_name']]
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            mock_config.project_root = temp_project_dir
            result = suggest_test_cases(str(source_file), function_name="nonexistent_function")
...
<coverage>
    ...
    def test_get_test_coverage_info_binary_format(self, temp_project_dir):
        ...
        mock_cov = MagicMock()
        mock_cov.get_data.return_value.measured_files.return_value = [str(temp_project_dir / "example.py")]
...
''')
        ...
        for suggestion in result['test_suggestions']:
            if suggestion.get('suggested_assertions'):
                pytest_assertions = ['assert', 'assert ==', 'assert !=', 'assert is', 'assert is not']
                has_pytest_assertion = any(
                    assertion in suggestion['suggested_assertions'] 
                    for assertion in pytest_assertions
                )
        ...
        assert has_any_pytest_assertion, "No suggestions found with pytest-style assertions"
...
''')
        ...
        for suggestion in result['test_suggestions']:
            if suggestion.get('suggested_assertions'):
                unittest_assertions = ['assertEqual', 'assertNotEqual', 'assertTrue', 'assertFalse', 'assertIs', 'assertIsNot']
                has_unittest_assertion = any(
                    assertion in suggestion['suggested_assertions'] 
                    for assertion in unittest_assertions
                )
        ...
        assert has_any_unittest_assertion, "No suggestions found with unittest-style assertions"
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            ...
            untested_methods = [f['name'] for f in untested['untested_functions']]
...
''')
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
            ...
            # Analyze test files
            analysis = analyze_test_files("tests")
            unittest_tests = [t for t in analysis['test_functions'] if t['framework'] == 'unittest']
...
class TestTestToolsErrorHandling:
    ...
    def test_functions_handle_permission_errors(self, temp_project_dir):
        ...
        with patch('redis_test_mcp_tools.tools.test_tools.find_test_files', side_effect=PermissionError("Permission denied")):
            with patch('redis_test_mcp_tools.tools.test_tools.config') as mock_config:
                mock_config.project_root = temp_project_dir
