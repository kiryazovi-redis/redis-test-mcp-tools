...
from redis_test_mcp_tools.tools.ast_tools import parse_module_ast, get_ast_from_file
...
"""),
        ...
        with patch('redis_test_mcp_tools.config.config.project_root', temp_project_dir):
            ...
            assert isinstance(result, dict), "Should return results despite syntax errors"
            assert 'error' not in result, "Should not fail entirely due to syntax errors"
    ...
    def test_ast_parsing_individual_failures(self, temp_project_dir):
        ...
        for filename, content in invalid_files:
            ...
            if 'error' in result:
                assert isinstance(result['error'], str), f"Error should be string for {filename}"
            else:
...
""")
        ...
        with patch('redis_test_mcp_tools.config.config.project_root', temp_project_dir):
            ...
            functions_with_types = [f for f in result['functions'] if f.get('return_type')]
            assert len(functions_with_types) > 0, "Should extract return type annotations"
    ...
    def test_import_analysis_failures(self, temp_project_dir):
        ...
        import_files = [
            ...
            ("missing_import.py", "from nonexistent_module import nothing\ndef test(): pass"),
    ...
    def test_partial_file_analysis(self, temp_project_dir):
        ...
        with patch('redis_test_mcp_tools.config.config.project_root', temp_project_dir):
            ...
            if 'test_functions' in result:
                valid_functions = [f for f in result['test_functions']]
                assert len(valid_functions) >= 2, "Should find functions from valid files"
