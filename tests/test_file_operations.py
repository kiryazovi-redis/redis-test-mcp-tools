...
from redis_test_mcp_tools.tools.test_tools import find_test_files
...
class TestFindTestFiles:
    ...
    def test_find_test_files_default_directory(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            assert len(result) >= 0
            test_file = next(
                (f for f in result if f["path"].endswith("test_module.py")), None
            )
...
class TestReadFileContent:
    ...
    def test_read_python_file(self, temp_project_dir):
        ...
        temp_project_dir / "src" / "module.py"
...
class TestGetDirectoryStructure:
                ...
                if child["type"] == "directory":
                    if "children" in child:
                        ...
                        assert len(child["children"]) == 0 or all(
                            "children" not in grandchild
                            or len(grandchild["children"]) == 0
    ...
    def test_get_directory_structure_error_handling(self, temp_project_dir):
        ...
        with patch("redis_test_mcp_tools.config.config.project_root", temp_project_dir):
            ...
            assert any(
                phrase in error_msg
                for phrase in ["not a directory", "is not a dir", "file"]
            ...
            assert (
                len(result["error"]) > 0
            ), "Should have meaningful error for invalid path"
